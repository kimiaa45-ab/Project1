...
def AR_model(
    ...
    for idx in range(batch_size):
        ...
        print(
            f"Instance {idx}: ems[idx] type: {type(ems[idx])}, length: {len(ems[idx])}, sample: {list(ems[idx].items())[:2] if isinstance(ems[idx], dict) else ems[idx][:2]}"
        )
        ...
        try:
            ...
            if isinstance(ems[idx], dict):
                items = list(ems[idx].values())[:charnum_node]  # Extract values
            else:
                items = ems[idx][:charnum_node]  # Assume list
            for item in items:
                if isinstance(item, (list, torch.Tensor, np.ndarray)):
                    node_embeddings.append(item)
                elif isinstance(item, str):
                    # Assume item is a node ID; map to characteristics
                    node = next(
                        (n for n in nodes[idx] if str(n["nodeID"]) == item), None
                    )
                    if node:
                        embedding = [
                        ...
                        print(
                            f"Warning: Instance {idx}: Node ID {item} not found in nodes"
                        )
                else:
                    print(f"Warning: Instance {idx}: Invalid item type {type(item)}")
        ...
        if len(node_embeddings) != charnum_node:
            print(
                f"Warning: Instance {idx}: Expected {charnum_node} node embeddings, got {len(node_embeddings)}"
            )
        ...
        if WNK.ndim > 2:
            WNK = WNK.squeeze()
        if WNK.ndim != 2 or WNK.shape[0] != node_embeddings_tensor.shape[1]:
            print(
                f"Error: Instance {idx}: WNK shape {WNK.shape} incompatible with node embeddings shape {node_embeddings_tensor.shape}"
            )
            placements.append(placement)
            continue
        ...
        if connectivity_array.ndim != 3:
            print(
                f"Error: Instance {idx}: connectivity must be 3D, got shape {connectivity_array.shape}"
            )
            placements.append(placement)
            continue
        if (
            connectivity_array.shape[0] < charnum_node
            or connectivity_array.shape[1] < charnum_node
        ):
            print(
                f"Error: Instance {idx}: connectivity shape {connectivity_array.shape} too small for {charnum_node} nodes"
            )
            placements.append(placement)
            continue
        connectivity_matrix = torch.tensor(
            connectivity_array[:charnum_node, :charnum_node, 0],
            dtype=torch.float32,
            device=device,
        )
        ...
        for z, (service_id, service_components) in enumerate(emc[idx].items(), 1):
            ...
            if not service:
                print(
                    f"Warning: Instance {idx}: No service found for ID {service_number}, skipping."
                )
                continue
        ...
        for idx_invalid in invalid_components:
            print(
                f"Warning: Instance {idx}: No nodes available for {component_ids[idx_invalid]}, using resource scores"
            )
            resource_scores = [
                max(
                    S[str(nodes[idx][j]["nodeID"])]["cpu"]
                    - component_specs[idx_invalid]["cpu"],
                    0,
                )
                + max(
                    S[str(nodes[idx][j]["nodeID"])]["memory"]
                    - component_specs[idx_invalid]["memory"],
                    0,
                )
                + max(
                    S[str(nodes[idx][j]["nodeID"])]["disk"]
                    - component_specs[idx_invalid]["disk"],
                    0,
                )
                for j in range(charnum_node)
            ]
            selected_nodes[idx_invalid] = torch.argmax(
                torch.tensor(resource_scores, device=device)
            )
